  ...
  return (
    <div style={{ minHeight:"100vh", background:"#09090a", color:"#e8e0d4", fontFamily:"Georgia,'Times New Roman',serif" }}>
      ...
      {showSettings && <Settings s={settings} set={setSettings} onClose={()=>setShowSettings(false)} />}
          ...
          {/* Loading state */}
          {busy && !shot && (
            <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", height:"80%", gap:20, animation:"fadeIn .3s ease" }}>
              <div style={{ position:"relative", width:56, height:56 }}>
                <div style={{ position:"absolute", inset:0, borderRadius:"50%", border:"2px solid rgba(200,160,80,.08)" }} />
                <div style={{ position:"absolute", inset:0, borderRadius:"50%", border:"2px solid transparent", borderTopColor:"#c8a050", animation:"spin 1s linear infinite" }} />
                <div style={{ position:"absolute", inset:10, borderRadius:"50%", border:"2px solid transparent", borderTopColor:"rgba(200,160,80,.35)", animation:"spin .7s linear infinite reverse" }} />
              </div>
              <div style={{ textAlign:"center", display:"flex", flexDirection:"column", gap:8 }}>
                <div style={{ fontSize:12, color:"#c8a050", fontFamily:"sans-serif", letterSpacing:".15em", textTransform:"uppercase", animation:"pulse 1.5s ease infinite" }}>
                  {genBusy ? "Claude writing prompts…" : "Rendering frames…"}
                </div>
                <div style={{ fontSize:10, color:"rgba(232,224,212,.3)", fontFamily:"sans-serif", fontStyle:"italic" }}>
                  {genBusy ? "Applying Visual Bible and frame descriptions" : "Generating via " + (activeRenderMode === "comfy" ? "ComfyUI" : "quick preview")}
                </div>
              </div>
            </div>
