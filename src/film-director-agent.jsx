...
4. DIRECTOR FEEDBACK (optional) — apply this feedback. Keep everything not criticised. Only change what feedback addresses.
...
const PREVIEW_PROVIDERS = {
  gemini: {
    ...
    models: [{ id: "gemini-3.1-flash-image-preview", label: "Gemini 2.0 Flash", desc: "Fast, high quality" }],
  },
  nanobanana: {
...
export default function App() {
  ...
  async function renderFrames(parsed, mode) {
    ...
    // For ComfyUI run sequentially (queue), for preview run in parallel
    let s1, s2;
    if (mode === "comfy") {
      s1 = await renderFrame(parsed.startFrame, setStartImg, setStartLoading, setStartMsg, setStartErr);
      s2 = await renderFrame(parsed.endFrame,   setEndImg,   setEndLoading,   setEndMsg,   setEndErr);
    } else {
