...
CAMERA LOCK — use the CAMERA LOCK field exactly as written in both frames. It overrides any camera description in the frame inputs. Both frames share identical camera position, height, angle, and focal length unless the Camera Lock explicitly describes movement.
...
LAYER 1 — POSITIVE MATERIAL LANGUAGE: Extract every approved material and texture. Only these may appear.
LAYER 2 — NEGATIVE MATERIAL CONSTRAINT: Extract every prohibited material. Use as hard negative prompt exclusions AND avoid in positive prompts.
...
□ Every named element translated through bible's material system?
...

{
  ...
  "sharedContext": "camera, lighting, environment, costume locked across both frames in the bible material language",
...
// ─────────────────────────────────────────────────────────────────────────────
// COMFYUI WORKFLOW — Flux.1 dev fp8
// ─────────────────────────────────────────────────────────────────────────────
function buildComfyWorkflow({ positivePrompt, negativePrompt, modelName, width, height, steps, seed }) {
  return {
    ...
    "4": { "inputs": { "text": negativePrompt || "blurry, deformed, low quality, watermark", "clip": ["1", 1] }, "class_type": "CLIPTextEncode" },
...
function injectIntoCustomWorkflow(workflowJson, positivePrompt, negativePrompt) {
  ...
  for (const [id, node] of Object.entries(wf)) {
    if (node.class_type === "KSampler" || node.class_type === "KSamplerAdvanced") {
      posId = node.inputs?.positive?.[0];
      negId = node.inputs?.negative?.[0];
      break;
    }
  }
  if (posId && wf[posId]) wf[posId].inputs.text = positivePrompt;
  if (negId && wf[negId]) wf[negId].inputs.text = negativePrompt;
  return wf;
}
...
async function comfyPoll(serverUrl, promptId) {
  for (let i = 0; i < 120; i++) {
    ...
    if (entry.status?.status_str === "error") throw new Error("ComfyUI render failed");
  }
...
// ─────────────────────────────────────────────────────────────────────────────
async function geminiGenerateImage(apiKey, prompt, aspectRatio, refImageBase64, refMimeType, seed) {
...
// ─────────────────────────────────────────────────────────────────────────────
const Spin = ({ size=12 }) => (
  <span style={{ display:"inline-block", width:size, height:size, border:"2px solid rgba(200,160,80,.18)", borderTopColor:"#c8a050", borderRadius:"50%", animation:"spin .8s linear infinite", flexShrink:0 }} />
);
...
// ─────────────────────────────────────────────────────────────────────────────
// STATUS BAR — shown in left panel during generation
// ─────────────────────────────────────────────────────────────────────────────
function StatusBar({ genBusy, renderBusy, activeRenderMode, providerLabel }) {
  const busy = genBusy || renderBusy;
  if (!busy) return null;
  return (
    <div style={{ borderRadius:8, border:"1px solid rgba(200,160,80,.25)", background:"rgba(200,160,80,.07)", overflow:"hidden", animation:"fadeIn .2s ease" }}>
      {/* Shimmer progress bar */}
      <div style={{ height:3, background:"rgba(200,160,80,.1)", position:"relative", overflow:"hidden" }}>
        <div style={{ position:"absolute", top:0, left:"-100%", width:"60%", height:"100%", background:"linear-gradient(90deg, transparent, rgba(200,160,80,.6), transparent)", animation:"shimmer 1.3s ease infinite" }} />
      </div>
          ...
          <div style={{ fontSize:10, color:"rgba(232,224,212,.35)", fontFamily:"sans-serif", marginTop:3 }}>
            {genBusy
              ...
              : activeRenderMode === "comfy"
                ? "Sent to ComfyUI — polling for result…"
                : "Generating start + end frames in parallel"}
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{ position:"absolute", inset:0, background:"#0f0f11", borderLeft:"1px solid rgba(255,255,255,.07)", display:"flex", flexDirection:"column", animation:"slideIn .25s ease", zIndex:10 }}>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"16px 18px", display:"flex", flexDirection:"column", gap:12 }}>
        {messages.length === 0 ? <div style={{ textAlign:"center", padding:"40px 0", color:"rgba(232,224,212,.2)", fontSize:12, fontFamily:"sans-serif", fontStyle:"italic" }}>No messages yet.</div>
          : messages.map((msg, i) => {
        ...
        <div style={{ padding:"18px 22px", display:"grid", gap:20, flex:1 }}>
          ...
          <div style={{ padding:"14px 16px", background:"rgba(255,255,255,.02)", border:"1px solid rgba(255,255,255,.07)", borderRadius:10 }}>
            ...
            <div style={{ display:"grid", gap:10 }}>
              ...
              {/* Per-provider model selection (not for Gemini — single model) */}
              ...
              {/* fal key always available for reference uploads */}
              {s.previewProvider !== "fal" && (
                <div>
                  <SLabel>fal.ai Key <span style={{ color:"rgba(232,224,212,.3)", fontWeight:400, textTransform:"none", letterSpacing:0 }}>— for reference image uploads</span></SLabel>
                  <SecretInput value={s.falKey} onChange={v => upd("falKey", v)} placeholder="fal_…" />
                </div>
              )}
            </div>
...
export default function App() {
  ...
  const [camera, setCamera] = useState(() => { try { return localStorage.getItem("da_camera") || ""; } catch(e) { return ""; } });
  ...
  useEffect(() => { try { localStorage.setItem("da_camera", camera); } catch(e) {} }, [camera]);
  ...
  const [activeRenderMode, setActiveRenderMode] = useState(null);
  ...
  const busy             = genBusy || renderBusy;
  const providerLabel    = PREVIEW_PROVIDERS[previewProvider]?.label || "preview";
  ...
  async function buildPrompts(feedback = null) {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Claude API error ${res.status}: ${errText.slice(0, 100)}`);
    }
    ...
    try {
      ...
      console.warn("JSON parse failed, attempting repair. Length:", clean.length, "Error:", parseErr.message);
      ...
      try {
        ...
        throw new Error("Response was truncated. Try shortening your frame descriptions.");
      }
    }
  }
  ...
  async function renderFrames(parsed, mode) {
    ...
    const renderFrame = async (frame, setImg, setLoad, setMsg, setErr, refResult) => {
      try {
        let url;
        if (mode === "comfy") {
          ...
          const wf = customWf
            ...
            : buildComfyWorkflow({ positivePrompt: frame.prompt, negativePrompt: frame.negativePrompt, modelName: comfyModel, width: comfyWidth, height: comfyHeight, steps: comfySteps });
        ...
        const msg = e.message.includes("401") ? "Invalid API key"
          : e.message.includes("402") ? "Insufficient credits"
          : e.message.includes("Cannot reach") ? "Cannot reach ComfyUI — is it running?"
          : e.message.slice(0, 80);
        setErr(msg); return null;
      } finally { setLoad(false); }
    };

    // For ComfyUI: sequential (queue). For Gemini: Frame 1 first, then Frame 2 with Frame 1 as reference.
    // For other preview providers: parallel.
    let s1, s2;
    if (mode === "comfy") {
  ...
  async function postToWeavy(parsed, s1, s2, ver) {
    ...
    try {
      const uid = `da-${parsed.sceneSlug || Date.now()}`;
      setWeavyAppUid(uid);
      ...
      <div style={{ display:"grid", gridTemplateColumns:"420px 1fr", minHeight:"calc(100vh - 50px)", position:"relative", zIndex:2 }}>
        ...
        <div style={{ borderRight:"1px solid rgba(255,255,255,.055)", padding:"22px 20px 120px", overflowY:"auto", maxHeight:"calc(100vh - 50px)", position:"sticky", top:50, display:"flex", flexDirection:"column", gap:18 }}>
          ...
          <div style={{ display:"grid", gap:9 }}>
            ...
            {/* Status bar — shows during any busy state */}
            {busy && (
            ...
            {!canGenerate && !busy && (
              <p style={{ fontSize:9, color:"rgba(232,224,212,.18)", fontFamily:"sans-serif", textAlign:"center", fontStyle:"italic" }}>
                {!claudeKey.trim() ? "Add your Claude API key in Settings"
                  ...
                  : !previewAvailable ? "Add a preview API key in Settings"
                  : ""}
              </p>
            )}
